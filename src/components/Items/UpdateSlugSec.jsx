...
export default function UpdateSlugSec({ cardData, openModal }) {
  const [isOpen, setIsOpen] = useState(false);
  const contentRef = useRef(null);
  const [height, setHeight] = useState(0);

  const slug = cardData?.slug || "Not set";
      ...
      >
        <div className="p-2">
          <div
            ...
            onClick={() => openModal("slug", "Update Slug")}
          >
        ...
        {slug === "Not set" && (
          <div className="text-center py-8 px-6">
            <div className="w-16 h-16 mx-auto bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mb-4">
              <FaLink className="w-8 h-8 text-gray-400 dark:text-gray-500" />
            </div>
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
              No Slug Set
            </h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              Add a slug to customize your public profile URL
            </p>
          </div>
        )}
      </div>
